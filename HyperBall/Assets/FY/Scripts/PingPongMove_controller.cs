...
public class PingPongMove_controller : MonoBehaviour {
    ...
    void Update()
    {
        if (!Riverse)
        {
            transform.localPosition = new Vector3(startPosition_X + Mathf.PingPong(Time.time * moveSpeed, moveDistance_X) - (moveDistance_X / 2.0f)
        ...
        {
            transform.localPosition = new Vector3(startPosition_X + -1*( Mathf.PingPong(Time.time * moveSpeed, moveDistance_X) - (moveDistance_X / 2.0f))
                                            , startPosition_Y + -1*( Mathf.PingPong(Time.time * moveSpeed, moveDistance_Y) - (moveDistance_Y / 2.0f))
                                            , startPosition_Z + -1*( Mathf.PingPong(Time.time * moveSpeed, moveDistance_Z) - (moveDistance_Z / 2.0f)));
        }
    }
}
